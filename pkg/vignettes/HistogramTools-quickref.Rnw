...
(EMDCC) are two error metrics for histograms.  The plots here show a
...
\hline
\texttt{HistToEcdf} & Return the cumulative distribution function of histogram \\
...
\texttt{SubsetHistogram}  & Return a new histogram with a subset of the buckets\\
\texttt{TrimHistogram}  & Return a new histogram with empty buckets
